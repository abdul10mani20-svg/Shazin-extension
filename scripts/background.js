try {
  importScripts("shared/browser-compat.js");
} catch (error) {}

function _hasSidePanel() {
  return !!(chrome.sidePanel && chrome.sidePanel.setPanelBehavior && chrome.sidePanel.open);
}
function _hasSidebarAction() {
  return !!(chrome.sidebarAction && chrome.sidebarAction.open);
}
...
function _openExtensionUi(param1) {
  ...
  if (_hasSidebarAction()) {
    try {
      const result = chrome.sidebarAction.open();
      if (result && typeof result.catch === "function") {
        return result.catch(() => chrome.tabs.create({ url: chrome.runtime.getURL("index.html") }));
      }
      return result;
    } catch (error) {}
  }
