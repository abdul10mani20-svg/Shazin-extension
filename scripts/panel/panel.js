(function () {
  ...
  let value452 = null;
  let flag9 = false;
  let euRuntimeRefreshTimer = null;
  let euOperationBlocked = false;
  let euUpgradeRedirectUrl = "";
  let euExtensionV6 = {
    enabled: true,
    usageGuideMarkdown: "",
    activationPageMarkdown: "",
    features: {
  ...
  function spIgnoreExpectedRuntimeError() {
    try {
      void chrome.runtime.lastError;
    } catch (error) {}
  }
  function spSafeTabSendMessage(tabId, message, callback) {
    try {
      chrome.tabs.sendMessage(tabId, message, (response) => {
        spIgnoreExpectedRuntimeError();
    ...
    chrome.tabs.query(
      ...
      (param223) => {
        if (
          ...
          spSafeTabSendMessage(param223[0].id, {
          ...
          chrome.tabs.query(
            ...
            (param224) => {
              (param224 || []).forEach((param225) => {
                try {
                  spSafeTabSendMessage(param225.id, {
                    action: "qlActivateBypass",
                  });
                } catch (error62) {}
              });
  ...
  function euApplyV5FeatureFlags() {
    chrome.storage.local.get(
      ["eu_extension_v6", "ql_extension_v6", "eu_extension_v5", "ql_extension_v5"],
      (param237) => {
      const config =
        param237.eu_extension_v6 ||
        param237.ql_extension_v6 ||
        param237.eu_extension_v5 ||
        param237.ql_extension_v5 ||
        {};
      ...
      setVisible("#sp-download-project", features.projectDownload !== false);
      setVisible("#sp-remove-watermark", features.removeWatermark !== false);
      setVisible("#sp-approve-plan", features.approvePlan !== false);
      setVisible("#sp-prompt-composer,#sp-msg,#sp-send,.sp-chat-input,.sp-chat-send", features.chat !== false);
      },
    );
  }
  function euGetV5FeatureFlags() {
    return new Promise((resolve) => {
      chrome.storage.local.get(
        ["eu_extension_v6", "ql_extension_v6", "eu_extension_v5", "ql_extension_v5"],
        (items) => {
          const config =
            items.eu_extension_v6 ||
            items.ql_extension_v6 ||
            items.eu_extension_v5 ||
            items.ql_extension_v5 ||
            {};
  ...
  function euRenderOperationBlock(operations) {
    ...
    if (!kind) {
      if (euOperationBlocked) {
        euOperationBlocked = false;
        euUpgradeRedirectUrl = "";
        function39();
      }
      return false;
    }
    euOperationBlocked = true;
    const body = document.getElementById("sp-body");
    if (body) {
      body.innerHTML = window.EUBackend.renderBlockPage(kind, operations);
    }
    const upgrade = operations.forceUpgrade || {};
    if (kind === "upgrade" && upgrade.autoRedirect && upgrade.downloadUrl && euUpgradeRedirectUrl !== upgrade.downloadUrl) {
      euUpgradeRedirectUrl = upgrade.downloadUrl;
      setTimeout(() => {
        window.location.href = upgrade.downloadUrl;
      }, 1200);
    }
    return true;
  }
  function euUpdateNotificationBadge(notifications) {
    const items = Array.isArray(notifications) ? notifications : [];
    chrome.storage.local.get(["ql_read_notifs"], (stored) => {
      const read = stored.ql_read_notifs || [];
  ...
  async function euRefreshRuntimeConfig() {
    try {
      const runtime = await window.EUBackend.getExtensionV5Runtime();
      const extensionV5 = runtime.enabled === false
        ? { enabled: false, features: {} }
        : (runtime.extensionV5 || {});
      if (runtime.extensionV6 && typeof runtime.extensionV6 === "object") {
        euExtensionV6 = {
          ...runtime.extensionV6,
          enabled: runtime.extensionV6.enabled !== false,
          usageGuideMarkdown:
            typeof runtime.extensionV6.usageGuideMarkdown === "string"
              ? runtime.extensionV6.usageGuideMarkdown
              : "",
        };
      }
      const operations = runtime.operations || {
        forceUpgrade: extensionV5.forceUpgrade || {},
        maintenance: extensionV5.maintenance || {},
      };
      chrome.storage.local.set({
        eu_extension_v5: extensionV5,
        ql_extension_v5: extensionV5,
        eu_extension_v6: euExtensionV6,
        ql_extension_v6: euExtensionV6,
        eu_operations: operations,
        ql_operations: operations,
      }, () => {
        if (!euRenderOperationBlock(operations)) {
          euApplyV5FeatureFlags();
          euRefreshUsageGuide();
          euMaybeShowOptionalUpgrade(operations);
        }
      });
      euUpdateNotificationBadge(runtime.notifications);
      if (value454 && value454.style.display !== "none") {
        function26();
      }
    } catch (error) {
      console.warn("[SP] Runtime config refresh failed:", error?.message || error);
    }
  }
  function euStartRuntimeRefresh() {
    if (euRuntimeRefreshTimer) clearInterval(euRuntimeRefreshTimer);
    euRefreshRuntimeConfig();
    euRuntimeRefreshTimer = setInterval(euRefreshRuntimeConfig, 30000);
  }
  ...
  const valueBackToPopup = document.getElementById("sp-back-to-popup");
  if (valueBackToPopup) {
    valueBackToPopup.addEventListener("click", () => {
      try {
        chrome.storage.local.set({
          ql_sidebar_mode: false,
        });
  ...
  const valueThemeButton = document.querySelector(".sp-theme-btn");
  if (valueThemeButton) {
    valueThemeButton.addEventListener("click", () => {
  ...
  document.querySelector(".sp-logout-btn").addEventListener("click", () => {
    ...
    chrome.tabs.query(
      ...
      (param239) => {
        if (param239[0]) {
          spSafeTabSendMessage(param239[0].id, {
    ...
    function32(function () {
      ...
      if (!flag9) {
        ...
        chrome.storage.onChanged.addListener((param255) => {
          if (
            param255.lovable_projectId ||
            param255.lovable_token ||
            param255.lovable_last_sync_ok ||
            param255.lovable_last_sync_status ||
            param255.lovable_last_sync_error
          ) {
            function42();
          }
        });
  ...
  function function40() {
    ...
    value498.innerHTML =
      '<div class="sp-log" id="sp-log"></div><span class="sp-shortcuts-title">QUICK SHORTCUTS</span><div class="sp-shortcuts-grid"><button id="sp-remove-watermark" class="sp-chip sp-utility-chip">🚫 <span>Remove Badge</span></button><button id="sp-download-project" class="sp-chip sp-utility-chip">⬇ <span>Download Project</span></button><button id="sp-approve-plan" class="sp-chip sp-utility-chip">✅ <span>Approve Plan</span></button></div><div id="sp-download-status" class="sp-log" style="display:none"></div>' +
      spUsageGuideHtml();
    function52();
    setupSpApprovePlanShortcut();
    chrome.storage.local.set({
  ...
  function function42() {
    chrome.storage.local.get(
      [
        "lovable_projectId",
        "lovable_token",
        "lovable_last_sync_ok",
        "lovable_last_sync_status",
        "lovable_last_sync_error",
      ],
      (param262) => {
        ...
        if (param262.lovable_projectId && param262.lovable_token) {
          const projectLabel = param262.lovable_projectId.substring(0, 6) + "...";
          if (param262.lovable_last_sync_ok === true) {
            value507.className = "sp-sync-status sp-sync-ok";
            value507.textContent = t("sync.ok") + " " + t("sync.project") + " " + projectLabel;
          } else if (param262.lovable_last_sync_status) {
            value507.className = "sp-sync-status sp-sync-waiting";
            value507.textContent =
              "Local only: " +
              projectLabel +
              " (" +
              String(param262.lovable_last_sync_status) +
              (param262.lovable_last_sync_error ? ", " + param262.lovable_last_sync_error : "") +
              ")";
          } else {
            value507.className = "sp-sync-status sp-sync-waiting";
            value507.textContent = "Captured locally: " + projectLabel;
          }
        } else {
          value507.className = "sp-sync-status sp-sync-waiting";
    ...
    value546.addEventListener("change", async () => {
      ...
      for (const value555 of value547) {
        ...
        try {
          ...
          console.warn("[EK] Upload failed:", error80.message);
          items13[value560].uploading = false;
  ...
  function function52() {
    ...
    value565.addEventListener("click", async function () {
      var value566 = document.getElementById("sp-log");
	    ...
	    value565.textContent = "⏳ processing...";
	    try {
	      await new Promise(function (param281, param282) {
	        chrome.runtime.sendMessage(
	          {
	            action: "backendRemoveWatermark",
	          },
            function (param283) {
              if (chrome.runtime.lastError) {
        ...
        spShowErrorLog(value566, "✗ ", error81);
      } finally {
        value565.disabled = false;
        value565.textContent = "🚫 Remove Badge";
      }
    });
  }
  async function function53() {
    ...
    const valueFeatureFlags = await euGetV5FeatureFlags();
    if (valueFeatureFlags.chat === false) {
      value570.className = "sp-log sp-log-error";
      value570.textContent = "Chat is disabled by admin.";
      return;
    }
    if (!value568) {
      value570.className = "sp-log sp-log-error";
    ...
    const valueAttachmentFiles = value574.map(function (param287) {
      return {
        file_id: param287.file_id || "",
        name: param287.file_name || "attachment",
        mime_type: param287.file_type || "application/octet-stream",
        size:
          param287.rawFile && Number.isFinite(param287.rawFile.size)
            ? param287.rawFile.size
            : 0,
        url: param287.public_url,
        public_url: param287.public_url,
      };
    });
    const valueOptimisticImageUrls = value574
      .filter(function (param287) {
        return (
          /^image\//i.test(param287.file_type || "") &&
          /^https:\/\//i.test(param287.public_url || "")
        );
      })
    ...
    try {
      ...
      if (!value580) {
        ...
        const value582 = value581[0] && value581[0].url ? value581[0].url : "";
        const value583 = value582.match(/\/projects\/([a-f0-9-]{36})/i);
        if (value583) {
          value580 = value583[1];
      ...
      if (!value580) {
        value570.className = "sp-log sp-log-error";
        value570.textContent = "⚠ Please open a project first, then try again.";
        value571.disabled = false;
        value571.textContent = "Send";
        return;
      }
        ...
        chrome.runtime.sendMessage(
          ...
          function (param292) {
            ...
            if (param292 && param292.ok) {
              ...
              param291(
                new Error(
                  (param292 && param292.error) || "Backend send failed",
                ),
              );
            }
          },
      ...
      spShowErrorLog(value570, "✗ ", error82);
    ...
    value607.addEventListener("click", async function () {
      ...
      try {
	        var value612 = await new Promise(function (param305) {
	          chrome.storage.local.get(
	            ["lovable_projectId", "eu_license_key", "ql_license_key", "lovable_email", "lovable_token"],
	            param305,
	          );
	        });
        ...
        if (!value617) {
          throw new Error("Please open a project first, then try again.");
	        ...
	        if (value608) {
          value608.textContent = "📡 Requesting project ZIP...";
        }
        value607.textContent = "📡 Downloading...";
        var value620 = value612.eu_license_key || value612.ql_license_key || "";
        if (!value620) {
          throw new Error("No license key");
        }
        var value621 = await fetch("https://io.eklas.dev/api/v1/lovable/download-zip", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "X-License-Key": value620,
          },
          body: JSON.stringify({
            licenseKey: value620,
            email: value612.lovable_email || "",
            token: value612.lovable_token || "",
            projectId: value617,
          }),
        });
        if (!value621.ok) {
          var value622 = await value621.json().catch(function () {
            return {};
          });
          throw new Error(value622.error || "API returned " + value621.status);
        }
        var value623 = await value621.blob();
        var value624 = value621.headers.get("content-disposition") || "";
        var value625 = value624.match(/filename\*=UTF-8''([^;]+)|filename="?([^"]+)"?/i);
        var value626 = value625 ? value625[1] || value625[2] || "" : "";
        try {
          value626 = decodeURIComponent(value626);
        } catch (error89) {}
        var value629 = new Date().toISOString().split("T")[0];
        var value630 = value626 || "lovable-" + value617.substring(0, 8) + "-" + value629 + ".zip";
        var value631 = URL.createObjectURL(value623);
        ...
        if (value608) {
          ...
          value608.textContent = "✅ Project ZIP downloaded successfully!";
        }
        value607.textContent = "✅ Download Complete!";
        setTimeout(function () {
          value607.textContent = "⬇ Download Project";
        ...
        if (value608) {
          spShowErrorLog(value608, "❌ ", error90);
          value608.style.display = "block";
        }
  ...
  function setupSpApprovePlanShortcut() {
    var value654 = document.getElementById("sp-approve-plan");
    if (!value654) {
      return;
    }
    value654.addEventListener("click", async function () {
      const value656 = document.getElementById("sp-log");
      value654.disabled = true;
      value654.textContent = "⏳ Sending...";
      try {
        if (value656) {
          value656.className = "sp-log sp-log-info";
          value656.innerHTML = SP_SVG.clock + " Sending approval...";
        }
        await new Promise(function (param328, param329) {
          chrome.runtime.sendMessage(
            {
              action: "backendApprovePlan",
            },
            function (param330) {
              if (chrome.runtime.lastError) {
                return param329(new Error(chrome.runtime.lastError.message));
              }
              if (param330 && param330.ok) {
                param328();
              } else {
                param329(
                  new Error(
                    (param330 && param330.error) || "Backend send failed",
                  ),
                );
              }
            },
        ...
        if (value656) {
          value656.className = "sp-log sp-log-success";
          value656.textContent = "✓ Plan Approved!";
        }
        function34("Approve Plan", "ok");
      } catch (error83) {
        spShowErrorLog(value656, "✗ ", error83);
        function34("Approve Plan", "error");
      } finally {
        value654.disabled = false;
        value654.textContent = "✅ Approve Plan";
      }
    });
  }

  const SP_DEFAULT_USAGE_GUIDE = `**We recommend backing up your projects regularly.**

## Correct Usage

1. After activating your license, open your project and **refresh the Lovable page** before sending your first prompt.
2. After sending a prompt, verify that the response shows **"Lovable"** instead of repeating your prompt text:
   - **✅ "Lovable" appears** = No credits used (free mode active)
   - **❌ Your prompt text** (for example, "Hello world") appears = Credits are being used. Refresh the page and resend the prompt.

## New Project

Creating a new project from the dashboard may consume **1 credit** (or less). Once the project is created, **all future prompts** inside that project are free (no credits used).

## Free Project Creation (No Credit Usage)

**Method to initialize a project without using credits:**

1. Start creating a new project from the dashboard.
2. **Immediately stop** the creation process as soon as it begins.
3. This initializes the project in free mode without deducting any credits.

## Troubleshooting

### Additional verification is required. Please complete the captcha and try again.
...
**Important:** The extension will not work with 0 credits. Keep at least 1 credit available in your active workspace.
...
Switch to a different Lovable workspace, or sign in with another account and create the project there.`;
  ...
  async function euLoadActivationPageContent() {
    try {
        ...
        euExtensionV6 = {
          ...
          usageGuideMarkdown:
            typeof runtime.extensionV6.usageGuideMarkdown === "string"
              ...
              : "",
          activationPageMarkdown:
            typeof runtime.extensionV6.activationPageMarkdown === "string"
              ? runtime.extensionV6.activationPageMarkdown
              : "",
        };
        chrome.storage.local.set({
          eu_extension_v6: euExtensionV6,
      ...
      await new Promise((resolve) => {
        chrome.storage.local.get(["eu_extension_v6", "ql_extension_v6"], (items) => {
          const cached = items.eu_extension_v6 || items.ql_extension_v6;
          if (cached && typeof cached === "object") {
            euExtensionV6 = { ...euExtensionV6, ...cached };
          }
          resolve();
        });
      });
  ...
  function spActivationInlineMarkdown(value) {
    return spEscapeHtml(value)
  ...
  function spActivationPageTextHtml() {
    if (euExtensionV6.enabled === false) return "";
    const markdown =
      typeof euExtensionV6.activationPageMarkdown === "string"
        ? euExtensionV6.activationPageMarkdown.trim()
        : "";
    if (!markdown) return "";
    ...
    lines.forEach((rawLine) => {
      ...
      if (heading) {
        closeList();
        html +=
          '<div class="sp-activation-heading">' +
          spActivationInlineMarkdown(heading[1]) +
          "</div>";
        return;
      }
      ...
      if (nextListType) {
        if (listType !== nextListType) {
          ...
          html += "<" + listType + ' class="sp-activation-list">';
        }
        html +=
          "<li>" +
          spActivationInlineMarkdown((ordered || unordered)[1]) +
          "</li>";
        return;
      }
      ...
      closeList();
      html +=
        '<p class="sp-activation-text">' +
        spActivationInlineMarkdown(line) +
        "</p>";
    });
  ...
  function euRefreshActivationPageText() {
    const container = document.getElementById("sp-activation-copy");
    if (!container) return;
    const html = spActivationPageTextHtml();
    container.className = html ? "sp-activation-copy" : "";
    container.innerHTML = html;
  }

  function spUsageInlineMarkdown(value) {
    return spEscapeHtml(value).replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>");
  }

  function spUsageGuideHtml() {
    if (euExtensionV6.enabled === false) return "";
    const markdown =
      typeof euExtensionV6.usageGuideMarkdown === "string" &&
      euExtensionV6.usageGuideMarkdown.trim()
        ? euExtensionV6.usageGuideMarkdown
        : SP_DEFAULT_USAGE_GUIDE;
    const lines = markdown.replace(/\r\n?/g, "\n").split("\n");
    let html = '<div class="sp-usage-guide"><div class="sp-usage-section">';
    let listType = "";
    ...
    lines.forEach((rawLine) => {
      ...
      if (section) {
        closeList();
        html +=
          '</div><div class="sp-usage-section"><div class="sp-usage-heading">' +
          spUsageInlineMarkdown(section[1]) +
          "</div>";
        return;
      }

      const issue = line.match(/^###\s+(.+)$/);
      if (issue) {
        closeList();
        html +=
          '<div class="sp-usage-issue-title">' +
          spUsageInlineMarkdown(issue[1]) +
          "</div>";
        return;
      }
      ...
      if (nextListType) {
        if (listType !== nextListType) {
          ...
          html +=
            ...
            listType +
            ' class="sp-usage-list' +
            (listType === "ul" ? " sp-usage-list-bullet" : "") +
            '">';
        }
        html +=
          "<li>" +
          spUsageInlineMarkdown((ordered || unordered)[1]) +
          "</li>";
        return;
      }

      closeList();
      html += '<p class="sp-usage-text">' + spUsageInlineMarkdown(line) + "</p>";
    });
  ...
  function euRefreshUsageGuide() {
    const existing = document.querySelector(".sp-usage-guide");
    const html = spUsageGuideHtml();
    if (existing) {
      if (html) {
    ...
    if (html && tabContent && text16 !== "history") {
      tabContent.insertAdjacentHTML("beforeend", html);
    }
  }

  function spShowErrorLog(el, prefix, error) {
    if (!el) return;
    const msg = spFormatErrorMessage(error);
    el.className =
      "sp-log sp-log-error" +
      (msg.indexOf("\n") !== -1 ? " sp-log-multiline" : "");
  ...
  function spFormatErrorMessage(error) {
    ...
    if (
      msg.includes('"type":"castle_denied"') ||
      msg.includes("suspicious activity")
    ) {
      return [
        "Project creation was blocked due to suspicious activity.",
        "",
