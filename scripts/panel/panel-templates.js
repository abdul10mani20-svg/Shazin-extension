...
function spTemplateMainUI(param201, param202) {
  return (
    ...
    t("btn.watermark") +
    '</span></button><button id="sp-download-project" class="sp-chip sp-utility-chip" data-i18n="btn.download">⬇ <span>' +
    t("btn.download") +
    '</span></button><button id="sp-approve-plan" class="sp-chip sp-utility-chip" data-i18n="btn.approvePlan">✅ <span>' +
    t("btn.approvePlan") +
    '</span></button></div><div id="sp-download-status" class="sp-log" style="display:none"></div>'
  );
}
function spTemplateStatusBadge(param203) {
  var valueStatusText = String(param203 || "Active").trim();
  var valueStatusClass =
    valueStatusText.toLowerCase() === "trial" ||
    valueStatusText.toLowerCase() === "test"
      ? "sp-badge-test"
      : "sp-badge-pro";
  return (
    ...
    valueStatusClass +
    '">' +
    spEscapeHtml(valueStatusText.toUpperCase()) +
    "</span>"
  );
}
...
function spTemplateNotifItem(param206) {
  var value417 =
    "en-US";
  const valueDateRaw = param206.created_at || param206.date || param206.startsAt || "";
  const valueDate = valueDateRaw ? new Date(valueDateRaw) : null;
  const value418 = valueDate && Number.isFinite(valueDate.getTime())
    ? valueDate.toLocaleDateString(value417)
    : "";
  const value419 = spSanitizeUrl(param206.link || param206.url);
  ...
  return (
    ...
    "</div>" +
    value420 +
    (value418 ? '<div class="sp-notif-item-date">' + value418 + "</div>" : "") +
    "</div>"
  );
}
